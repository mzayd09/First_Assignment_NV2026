...
test('TC-04: Dashboard page should display correctly', async ({AllActions}) =>{
    ...
    await expect(AllActions.dashboardAction.Dashboard.DashboardPageTitle).toHaveText(loginData.DashboardPageTitle);
});
...
test('TC-05: Dashboard menu should open on clicking Open Menu button', async ({AllActions}) =>{
    ...
    await expect(AllActions.dashboardAction.Dashboard.OpenMenuButton).toBeVisible();
    await expect(AllActions.dashboardAction.Dashboard.MenuAllItems).toHaveText(loginData["All Menu Items"]);   
});